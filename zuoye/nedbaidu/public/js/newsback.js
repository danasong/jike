$(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                data.forEach(function(item, index, array) {
                    ...
                    var $tdname = $("<td>").html(item.news_name);
                    var $tdtext = $("<td>").html(item.news_text);
                    var $tdimg = $("<td>").html(item.news_img);
                    var $tddate = $("<td>").html(item.news_date.substring(0, 10));
                    var $tdsource = $("<td>").html(item.news_source);
                    var $tdctrl = $("<td>");
    ...
    $("#submits").click(function(event) {
        ...
        if (!newsname.val() == "请选择" || newstext.val() == "" || newssuccess.val() == "" || newsdate.val() == "" || newsimg.val() == "") {
            ...
            var jsonNews = {
                newsname:Check($("#newsname").val()),
                newstext:Check($("#newstext").val()),
                newssuccess:Check($("#newssuccess").val()),
                newsdate: Check($("#newsdate").val()),
                newsimg: Check($("#newsimg").val())
            }
            $.ajax({
                url: 'admin/inset',
                type: 'post',
    ...
    $("#upsubmit").click(function(e) {
        ...
        if (updateid != null) {
            $.ajax({
                ...
                dataType: 'json',
                success: function(data) {
                    console.log(data.affectedRows > 0 ? "成功" : "失败");
